...
join ORDER_DETAILS c on c.order_id = a.id
...
begin
	begin tran
		begin try
			insert into USERS values (@country, 2, @first_name, @last_name, @company_name, @username, @password, @phone, @email, null, getdate(), getdate(), 1)
...
---
go
create or alter proc SP_INSERT_ORDER @shipping_id int, @country_id int, @coupon_id int, @user_id int, @note nvarchar(max) as
begin
	begin tran
		begin try
			insert into ORDERS values (@shipping_id , @country_id , @coupon_id, 4, @user_id , @note, null, getdate(), getdate(), 0)
			commit transaction
		end try
		begin catch
			rollback transaction
